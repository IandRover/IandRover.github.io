...
## Connecting uniformity and reduncancy Minimization
Consider $X=[n_1, n_2,..., n_N]^T$ be some data points located on $S^1$. Now we want to maximize the **square of distance** between each data points: $\max \sum_{i,j} |n_i-n_j|^2_2$.
<!-- Clearly from definition we have $tr(X^T X)=N$. -->


$\max \sum_{i,j} |n_i-n_j|^2_2 \\
=\max \sum_{i,j} (n_i^2+n_j^2-2n_i^T \cdot n_j) \\
=\min \sum_{i} n_i^T \cdot n_j \\
=\min (\sum_{i} n_i)^T \cdot (\sum_{j} n_j) \\
=\min \mu^2$
where $\mu=\sum_i n_i$

假設現在有$N$個小球，我要在$S^1$上面排列這些小球，使得這些小球彼此之間的距離是最大的。我的直覺上，這個問題的解就是"把球按照$\{\frac{2\pi}{N}, \frac{2\cdot2\pi}{N}, \frac{(N-1)\cdot2\pi}{N}\}$排列在$S^1$上面"。但是我無法證明為什麼這個是解。

以下是題目敘述，和簡單的identities。而我在寫下第二式之後，發現無法往下推。因為我能使用的工具，僅有柯西不等式，但它只能求maximum，所以無法用在這邊。

題目敘述：Consider $X=[n_1, n_2,..., n_N]^T$ be some data points located on $S^1$ and $n_i \in \mathbf{R}^2$. Now I can move the data points such that the overall **distance** between each data points is maximized: $\max \sum_{i,j} |n_i-n_j|_2$. One instinct is that the solution is "to unifomly arrange/distribute the data pionts on $S^1$". However, I current has no idea as how to prove that it is the solution of $\max \sum_{i,j} |n_i-n_j|_2$.

At the same time, it is clear that when the data is uniformly distributed on $S^1$", the off-diagonal entries of the cross-correlation of the data should be zero, which means that there is no correlation between the first dimension and the second dimension of data.

$\max \sum_{i,j} |n_i-n_j|_2 \\
=\max \sum_{i,j} \sqrt{(2-2n_i^T \cdot n_j)} \\
\sim \min \sum_{i,j} \sqrt{n_i^T \cdot n_j}$
